...
export class I18nController {
  ...
  updateUI() {
    ...
    if (headerTitle) headerTitle.textContent = this.t('title');
    
    const headerSubtitle = document.querySelector('.subtitle');
    if (headerSubtitle) headerSubtitle.textContent = this.t('subtitle');
    
    // Update transcription language selector
    const transcriptionLangLabel = document.querySelector('.language-selector label');
    if (transcriptionLangLabel) transcriptionLangLabel.textContent = this.t('transcriptionLanguageLabel');
    ...
    if (recordTab) recordTab.textContent = this.t('realTimeRecording');
    
    const uploadTab = document.querySelector('[data-tab="upload"] .tab-label');
    if (uploadTab) uploadTab.textContent = this.t('fileUpload');
    ...
    if (recordText) recordText.textContent = this.t('startRecording');
    
    // Update upload area
    const uploadText = document.querySelector('.upload-text');
    if (uploadText) uploadText.textContent = this.t('dragDropText');
    
    const uploadHint = document.querySelector('.upload-hint');
    if (uploadHint) uploadHint.textContent = this.t('supportedFormats');
    
    const transcribeButton = document.getElementById('transcribe-button');
    if (transcribeButton) transcribeButton.textContent = this.t('startTranscription');
    
    const fileInput = document.getElementById('file-input');
    if (fileInput) fileInput.setAttribute('aria-label', this.t('selectAudioFile'));
    
    const uploadArea = document.getElementById('upload-area');
    if (uploadArea) uploadArea.setAttribute('aria-label', this.t('selectAudioFile'));
    
    // Update results section
    const resultsTitle = document.querySelector('.results-header h3');
    if (resultsTitle) resultsTitle.textContent = this.t('transcriptionResults');
    ...
    if (placeholder) placeholder.textContent = this.t('resultsPlaceholder');
    
    const loadingText = document.getElementById('loading-text');
    if (loadingText) loadingText.textContent = this.t('processing');
    
    const transcriptionResult = document.getElementById('transcription-result');
    if (transcriptionResult) transcriptionResult.setAttribute('aria-label', this.t('transcriptionContent'));
    
    // Update footer
    const footerText = document.querySelector('.app-footer p');
    if (footerText) footerText.textContent = this.t('privacyNote');
    
    // Update aria labels
    const tabContainer = document.querySelector('.tab-container');
    if (tabContainer) tabContainer.setAttribute('aria-label', this.t('functionSelection'));
  }
